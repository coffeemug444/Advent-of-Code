...
int main()
{
   ...
   for (std::string_view command : lines)
   {
      ...
      std::vector<char>& from_stack = box_stacks.at(from);
      std::vector<char>& to_stack = box_stacks.at(to);
      to_stack.insert(to_stack.end(), from_stack.end() - count, from_stack.end());
      from_stack.erase(from_stack.end() - count, from_stack.end());
   }
